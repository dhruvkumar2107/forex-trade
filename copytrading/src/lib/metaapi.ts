...
class MetaApiService {
  ...
  async getAccountInfo(accountId: string): Promise<MetaApiAccount | null> {
    try {
      ...
      console.error('[MetaApi] Failed to get account info:', error);
      return null;
    }
  }
  ...
  async getOpenTrades(accountId: string): Promise<MetaApiTrade[]> {
    try {
      ...
      console.error('[MetaApi] Failed to get open trades:', error);
      return [];
    }
  }

  async getTradeHistory(accountId: string, startTime?: string): Promise<MetaApiTrade[]> {
    try {
      const url = startTime
        ? `${this.baseUrl}/users/current/accounts/${accountId}/history?startTime=${startTime}`
        : `${this.baseUrl}/users/current/accounts/${accountId}/history`;
      const res = await fetch(url, { headers: this.getHeaders() });
      if (!res.ok) return [];
      return await res.json();
    } catch (error) {
      console.error('[MetaApi] Failed to get trade history:', error);
      return [];
    }
  }
  ...
  async copyTrade(
    ...
    try {
      ...
      console.error('[MetaApi] Failed to copy trade:', error);
  ...
  async closeTrade(accountId: string, tradeId: string): Promise<boolean> {
    try {
      ...
      console.error('[MetaApi] Failed to close trade:', error);
      return false;
    }
  }
  ...
  async modifyTrade(
    ...
    try {
      ...
      if (!res.ok) {
        return { success: false, error: data.message || 'Trade modification failed' };
      }

      return { success: true };
    } catch (error) {
      console.error('[MetaApi] Failed to modify trade:', error);
      return { success: false, error: 'Network error' };
    }
  }
